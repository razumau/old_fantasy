		...
		if (this.state.user.teams) {
			...
			return (React.createElement("div", { className: "userBox" },
				...
				React.createElement("div", {id:"selectionsLinkTeams"},
					React.createElement("a", {href:"https://fantasy.razumau.net/ochvr-2019/results", id: "selectionsLink"}, "Кого выбрали другие?"))
				)
			);
		} else {
			return (React.createElement("div", { className: "userBox" },
...
function authCallback(authData) {
	if (authData) {
		hideLoginButtons();
		userRef = rootRef.child("users-ochvr-2019").child(authData.uid);
...
function showTable() {
	$table.bootstrapTable({
		method: 'get',
		url: 'https://popping-inferno-4625.firebaseio.com/teams-ochvr-2019.json',
		cache: false,
		clickToSelect: true,
		columns: [{
			field: 'state',
