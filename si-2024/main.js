	...
	render: function() {
		...
		if (this.state.user.teams) {
			...
			return (React.createElement("div", { className: "userBox" },
				...
				React.createElement("p", null, "Изменения можно делать до 12:00 23 марта."),
				React.createElement("p", null, "Оценивал Валентин Копочель."),
...
function authCallback(authData) {
	if (authData) {
		hideLoginButtons();
		userRef = rootRef.child("users-si-2024").child(authData.uid);
...
function showTable() {
	$table.bootstrapTable({
		method: 'get',
		url: 'https://popping-inferno-4625.firebaseio.com/teams-si-2024.json',
		cache: false,
		clickToSelect: true,
		columns: [{
			field: 'state',
